...
void setup() {
  ...
  Rtc.SetDateTime(compiled); // Force update

  Serial.println("RTC time forced to compile time!");
...
void loop() {
  ...
  // Only read sensors and perform actions during watering times
  if (isWateringTime(now)) {
    ...
    if (!isPumping && !isInCountdown && !isWaiting && currentMillis - previousMillis >= checkInterval && waterRawVal >= 50) {
    ...
    //  to save power
    delay(5000);
    Serial.println("Not in watering time - power saving mode");
  }
...
bool isWateringTime(const RtcDateTime& dt) {
  ...
  Serial.println();
    ...
    // (6:00 AM - 8:59 AM) added 9 for testing
  bool isMorning = (hour == 6 && minute >= 0) || (hour == 7) || (hour == 8 && minute <= 59) || (hour == 9 && minute <= 13);

  //  (5:00 PM - 7:59 PM)
  bool isEvening = (hour == 17 && minute >= 0) || (hour == 18) || (hour == 19 && minute <= 59);
